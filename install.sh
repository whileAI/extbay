...
EXTBAY_IMAGE="${EXTBAY_IMAGE:-ghcr.io/whileai/extbay:0.1.0}"
...
[ -n "$candidates" ] || die "no running Portainer CE container was found"
count="$(printf '%s\n' "$candidates" | wc -l | tr -d ' ')"
[ "$count" = 1 ] || { printf '%s\n' "$candidates" >&2; die "multiple Portainer containers found; stop unrelated instances or set up ExtBay manually"; }
...
if ! docker image inspect "$EXTBAY_IMAGE" >/dev/null 2>&1; then
